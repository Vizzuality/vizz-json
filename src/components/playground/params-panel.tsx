...
import type { ParamConfig, ResolvedParams, InferredParam } from "#/lib/types";
...
function ColorPickerControl({ value, onChange }: ColorPickerControlProps) {
  const safeValue = typeof value === "string" && value.startsWith("#") ? value : "#000000";
...
function ParamControl({ inferred, currentValue, onChange }: ParamControlProps) {
  switch (inferred.control_type) {
    case "slider": {
      const numValue = typeof currentValue === "number" ? currentValue : (inferred.min ?? 0);
    ...
    case "color_picker": {
      const strValue = typeof currentValue === "string" ? currentValue : "#000000";
      return (
        <ColorPickerControl
          value={strValue}
    ...
    default: {
      ...
      return <span className="text-xs text-destructive">Unknown control: {String(_exhaustive)}</span>;
    }
  }
}
