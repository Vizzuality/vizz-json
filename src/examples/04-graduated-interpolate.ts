...
const config = {
  metadata: {
    title: 'Expression — interpolate',
    description:
      'Continuous color ramp for population using interpolate expression with parameterized stops',
    tier: 'intermediate',
  },
  config: {
    sources: [
    ...
    styles: [
      {
        ...
        filter: [
          'all',
          ['>=', ['get', 'pop_est'], '@@#params.threshold_1'],
          ['<=', ['get', 'pop_est'], '@@#params.threshold_3'],
        ],
        paint: {
          'fill-color': [
            'interpolate',
